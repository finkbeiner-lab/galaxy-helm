...
def multiprocess_montage():
    ...
    for i in xrange(len(input_image_stack_list)):
        job = cluster.submit(input_image_stack_list[i], HORIZONTAL_NUM_IMAGES, VERTICAL_NUM_IMAGES, PIXEL_OVERLAP, DTYPE, OUTPT_MONTAGED_PATH)
        job.id = i
        jobs.append(job)
...
if __name__ == '__main__':
    ...
    utils.save_user_args_to_csv(args, path_montaged_images, 'montage_distributed')
