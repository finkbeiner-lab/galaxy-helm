...
def get_image_tokens_list(input_bg_dir, robo_num, imaging_mode, valid_wells, valid_timepoints):
    ...
    if DIR_STRUCTURE == 'root_dir':
        image_paths = [os.path.join(input_bg_dir, name) for name in os.listdir(input_bg_dir) if
        ...
        relevant_dirs = [input_bg_dir] + [os.path.join(input_bg_dir, name) for name in os.listdir(input_bg_dir) if
                                          os.path.isdir(os.path.join(input_bg_dir, name))]
        image_paths = [os.path.join(relevant_dir, name) for relevant_dir in relevant_dirs for name in
                       os.listdir(relevant_dir) if name.endswith('.tif') and '_FIDUCIARY_' not in name]
    else:
...
def montage(image_stack_list):
    ...
    if len(image_stack_list) != HORIZONTAL_NUM_IMAGES * VERTICAL_NUM_IMAGES:
        print('[Error!!] Number of images(%d) DO NOT match Number of panels(%d)' % (len(image_stack_list),
                                                                                    HORIZONTAL_NUM_IMAGES
                                                                                    * VERTICAL_NUM_IMAGES))
        ...
        for idx_i, i in enumerate(range(HORIZONTAL_NUM_IMAGES)):
            ...
            for idx_j, j in enumerate(enumerate_list):
                ...
                if current_pannel is None:
                    raise Exception(
                        '%s is corrupted, please run Corrupted Images Detector to find out all the corrupted images.' % (
                            os.path.basename(image_stack_list[j][0])))
    ...
    output_img_location = utils.reroute_imgpntr_to_wells(os.path.join(OUTPT_MONTAGED_PATH, montaged_filename),
                                                         image_stack_list[0][3])
    # output_img_location = ''
    # if DIR_STRUCTURE == 'root_dir':
...
if __name__ == '__main__':
    ...
    parser.add_argument("--px_overlap",
                        dest="px_overlap", default='',
                        help="Overlap in pixels.")
    ...
    path_bgcorr_images = utils.get_path(args.input_image_path, var_dict['GalaxyOutputPath'], 'BackgroundCorrected')
    print('Input path: %s' % path_bgcorr_images)

    path_montaged_images = utils.get_path(args.output_results_path, var_dict['GalaxyOutputPath'], 'MontagedImages')
    print('Output path: %s' % path_montaged_images)

    if args.montage_order == 'autodetect':
        parent_dir = os.path.dirname(var_dict['InputPath'])
        if 'Robo3Images' in parent_dir or 'Robo4Images' in parent_dir:
            MONTAGE_ORDER = 'robo034'
        elif 'Robo5Images' in parent_dir:
            MONTAGE_ORDER = 'robo5'
        elif 'Robo6Images' in parent_dir:
            MONTAGE_ORDER = 'robo6'
        elif 'IXM' in parent_dir:
            MONTAGE_ORDER = 'ixm'
        else:
            raise Exception('Unable to auto-detect microscope to determine montage order. Please specify manually.')
    else:
        MONTAGE_ORDER = args.montage_order
    print('Montage order scheme: %s' % str(MONTAGE_ORDER))
    ...
    assert os.path.exists(path_bgcorr_images), 'Confirm the path for data exists (%s)' % path_bgcorr_images
    assert os.path.exists(path_montaged_images), 'Confirm the path for results exists (%s)' % path_montaged_images
    ...
    if args.px_overlap == '':
        ...
        PIXEL_OVERLAP = int(args.px_overlap)
        var_dict['ImagePixelOverlap'] = PIXEL_OVERLAP
    ...
    utils.save_user_args_to_csv(args, path_montaged_images, 'montage_mp' + '_' + timestamp)
