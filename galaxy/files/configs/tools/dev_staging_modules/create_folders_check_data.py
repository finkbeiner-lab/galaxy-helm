...
def parse_tokens(filenames, robo_num):
    ...
    if robo_num == 0:
        ...
        files.sort_values(['Well','Timepoint','Channel','Panel','BurstIndex','Zstep'], inplace = True)
    elif robo_num == 3:
        ...
        files.sort_values(['Well','Timepoint','Panel','Channel'], inplace = True)
...
def check_data(var_dict):
    ...
    if len(incomplete_wells) != 0:
        ...
        assert len(incomplete_wells) == 0, 'Dataset is missing images (also saved as csv in output directory):\n\n%s' % incomplete_data_output.to_string(index = False, index_names = False)
