...
def register_stack(image_stack_experiment_well, MORPHOLOGY_CHANNEL, ALIGNMENT_ALGORITHM, OUTPUT_ALIGNED_PATH, SHIFTS):     
        ...
        for idx, t in enumerate(sorted_morphology_timepoint_keys[:-1]):       
            ...
            for ix, item in enumerate(morphology_timepoint_dict[t]):
                ...
                # Apply the shift to the other iamges(zs, bursts) in current timepoint
                    ...
                    tif_output = TIFF.open(os.path.join(OUTPUT_ALIGNED_PATH, other_zb_image_filename.replace('.tif', '_ALIGNED.tif')), mode='w')
                ...
                for idx, t in enumerate(sorted_other_channel_timepoint_keys):  
                    ...
                    if t in pre_calculated_shift_dict:     
                        ...
                        for ix, item in enumerate(other_channel_timepoint_dict[t]):
                            ...
                            tif_output = TIFF.open(os.path.join(OUTPUT_ALIGNED_PATH, other_channel_image_filename.replace('.tif', '_ALIGNED.tif')), mode='w')
...
def variety_alignments():
    ...
    cluster = dispy.SharedJobCluster(register_stack, scheduler_node='fb-image-compute01')
    jobs = []

    for i in range(len(input_image_stack_list)):

        job = cluster.submit(input_image_stack_list[i], MORPHOLOGY_CHANNEL, ALIGNMENT_ALGORITHM, OUTPUT_ALIGNED_PATH, SHIFTS)
        job.id = i
        jobs.append(job)
    cluster.wait() # waits until all jobs finish
    # Get results from workers, and catch exception
    for map_job in jobs:
        rst = map_job() # waits for job to finish and returns results
        if rst is not None:            
            LOG_INFO.update(rst[0]) # update result to final log
            SHIFTS.update(rst[1])
        else:
