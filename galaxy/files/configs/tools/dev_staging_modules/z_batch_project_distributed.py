...
def get_image_tokens_list(input_dir, robo_num, imaging_mode, valid_wells, valid_timepoints):
    ...
    if DIR_STRUCTURE == 'root_dir':
        ...
        relevant_dirs = [input_dir] + [os.path.join(input_dir, name) for name in os.listdir(input_dir) if
                                       os.path.isdir(os.path.join(input_dir, name))]
...
def project_in_z(image_stack_list, SELECTED_CHANNELS, PROJECTION_TYPE, OUTPUT_PATH):
    import os
    import numpy as np
    import cv2
    from libtiff import TIFF
    ...
    if current_channel in SELECTED_CHANNELS and len(image_stack_list)>1:
        ...
        tif_output = TIFF.open(os.path.join(OUTPUT_PATH, rst_img_name), mode='w')
...
def multiprocess_project_in_z(): 
    ...
    cluster = dispy.SharedJobCluster(project_in_z, scheduler_node='fb-image-compute01')
    jobs = []
    for i in xrange(len(input_image_stack_list)):
        job = cluster.submit(input_image_stack_list[i], SELECTED_CHANNELS, PROJECTION_TYPE, OUTPUT_PATH)
        job.id = i
        jobs.append(job)
...
if __name__ == '__main__':
    ...
    utils.save_user_args_to_csv(args, OUTPUT_PATH, 'z_batch_project_distributed'+'_'+timestamp)
