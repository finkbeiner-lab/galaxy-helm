...
if __name__ == '__main__':
    ...
    print('Alignment algorithm: %s' % ALIGNMENT_ALGORITHM)

    INPUT_MONTAGED_PATH = utils.get_path(args.input_image_path, var_dict['GalaxyOutputPath'], 'MontagedImages')
    print('Input path: %s' % INPUT_MONTAGED_PATH)

    OUTPUT_ALIGNED_PATH = utils.get_path(args.output_results_path, var_dict['GalaxyOutputPath'], 'AlignedImages')
    print('Output path: %s' % OUTPUT_ALIGNED_PATH)
    ...
    if args.shift and args.shift.strip():
        print('Previous calculated shift file path: %s' % args.shift.strip())
    ...
    assert os.path.isdir(INPUT_MONTAGED_PATH), 'Confirm the path for input data exists (%s)' % INPUT_MONTAGED_PATH
    assert os.path.isdir(OUTPUT_ALIGNED_PATH), 'Confirm the path for output results exists (%s)' % OUTPUT_ALIGNED_PATH
    ...
    utils.save_user_args_to_csv(args, OUTPUT_ALIGNED_PATH, 'variety_alignments_mp'+'_'+timestamp)