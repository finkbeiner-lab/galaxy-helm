function [outIm,whatScale,Direction,ALLangles,ALLfiltered,l1,l2,allIx,allIy] = FrangiFilter2D(I, options)
...
defaultoptions = struct('FrangiScaleRange', [1 10], 'FrangiScaleRatio', 2, ...
...
if length(options.FrangiScaleRange) == 2 % default behavior from original code
    sigmas = options.FrangiScaleRange(1):options.FrangiScaleRatio:options.FrangiScaleRange(2);
elseif length(options.FrangiScaleRange) == 1 % only one scale requested
        sigmas = [options.FrangiScaleRange options.FrangiScaleRange];
else % assume we have the range specified here 
    sigmas = options.FrangiScaleRange;
end
...
l1 = zeros([size(I) length(sigmas)]);
l2 = zeros([size(I) length(sigmas)]);
allIx = zeros([size(I) length(sigmas)]);
allIy = zeros([size(I) length(sigmas)]);
...
for i = 1:length(sigmas),
    ...
    % -------------------------------------------------
    % See miccai-1998 paper by Frangi
    % The constant c is half the maximum hessian norm
    c = 0.5 * max( sqrt(S2(:)) );%disp(c);
    % -------------------------------------------------
