...
% 06/05/2015
function [csv_dir, timeStamp] = run_mito_analysis_longitudinal(montageFolder, maskFolder, varargin)
...
numTimeSteps = length(imfinfo(montage_image{1}));
...
for wellNumber = 1:numberOfImages % process per well
    fprintf('Image : %s\n', names{wellNumber});
    ...
    for timeStep = 1:numTimeSteps
        ...
        % calculate and record the average stats per well ****** AT THIS TIME POINT ******
    ...
    appendToCSV(csvTmpName, current_well, timeStampForCSV, ...
        well_circularity(wellNumber,1:numTimeSteps), well_skel_len(wellNumber,1:numTimeSteps), ...
        well_eccentricity(wellNumber,1:numTimeSteps), well_aspect_ratio(wellNumber,1:numTimeSteps), ...
        well_solidity(wellNumber,1:numTimeSteps), well_blob_count(wellNumber,1:numTimeSteps), ...
        [], timeStep);

    % write well data to it's own csv file
    appendToCSV(tmp_name, current_well, timeStampForCSV, ...
        well_circularity(wellNumber,1:numTimeSteps), well_skel_len(wellNumber,1:numTimeSteps), ...
        well_eccentricity(wellNumber,1:numTimeSteps), well_aspect_ratio(wellNumber,1:numTimeSteps), ...
        well_solidity(wellNumber,1:numTimeSteps), well_blob_count(wellNumber,1:numTimeSteps), [], timeStep);
...
resultFile = fullfile(csv_dir, sprintf('results-juliam-%s.mat', timeStamp));
save(resultFile, 'names', 'well_skel_len', 'well_circularity', 'well_solidity', ...
    'well_aspect_ratio', 'well_eccentricity', 'well_blob_count');
